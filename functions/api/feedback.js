...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      console.error('Token request failed', tokenRes.status, detail);
      return json({ error: 'Token request failed', detail }, 500);
    }

    const tokenData = await tokenRes.json();
    accessToken = tokenData.access_token;
  } catch (err) {
    return json({ error: 'Token request threw', detail: String(err) }, 500);
  }
  ...
  try {
    ...
    if (!sendRes.ok) {
      ...
      console.error('Send failed', sendRes.status, detail);
      return json({ error: 'Send failed', detail }, 500);
    }
  } catch (err) {
    return json({ error: 'Send threw', detail: String(err) }, 500);
  }
